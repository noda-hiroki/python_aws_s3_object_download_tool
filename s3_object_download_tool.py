...
def func_download_files(download_bucket_name: str, target_prefix: str) -> None:
    ...
    try:
        while True:
            #   アクセスキー、シークレットアクセスキー、リージョンを設定するか？
            select = input("Set your access key, secret access key, and region?(y/n):")
            if select in ("y", "n"):
                break
            else:
                continue
...
try:
    while True:
        #   プレフィックスを指定し、配下のファイルを全てダウンロードするか？
    ...
    if select_1 == "y":
        ...
        while True:
            #   アクセスキー、シークレットアクセスキー、リージョンを設定するか？
            select_2 = input("Set your access key, secret access key, and region?(y/n):")
            if select_2 in ("y", "n"):
                break
            else:
                continue
